...
export default function ProductCard({id, title, description, price, image, rating, reviews}:{id:string, title:string, description:string, price:string, image:string, rating:number, reviews:number}){
   ...
   return <div className="relative m-10 flex w-full max-w-sm flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md">
    <a className="relative mx-3 mt-3 flex h-60 overflow-hidden rounded-xl" href="#">
      <img className="object-cover" src={image} alt="product image" />
      <span className="absolute top-0 left-0 m-2 rounded-full bg-black px-2 text-center text-sm font-medium text-white">{Math.floor(Math.random()*100)}%OFF</span>
    </a>
    <div className="mt-4 px-5 pb-5">
      ...
      <div className="mt-2 mb-5 flex items-center justify-between">
        <p>
          ...
          <span className="text-sm text-slate-900 line-through">{Math.floor(Math.random()*1000)}</span>
        </p>
        <div className="flex items-center">
          
          <div className="flex items-center  mr-2 ml-3 rounded bg-green-500 px-2.5 py-0.5 text-xs font-semibold">
            <span>{rating} </span> 
            <svg aria-hidden="true" className="h-5 w-5 text-yellow-300" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
          </svg>
          </div>
          <span className="ml-1 w-fit rounded bg-yellow-400 px-2.5 py-0.5 text-xs font-semibold">{reviews} Ratings</span>
        </div>
      </div>
